package com.nttdata.transactionfixed.controller;
...
public class TransactionController {
  ...
  public Mono<ResponseEntity<Transaction>> create(
      @RequestBody Transaction transactionFixedTerm) {

    return service
        .findFixedTermById(transactionFixedTerm.getFixedTerm().getId())
        .flatMap(fixedTerm -> service
            .countTransactions(transactionFixedTerm.getFixedTerm().getId(),
                transactionFixedTerm.getTypeTransaction())
            .filter(count -> {
              Integer limit = 0;
              switch (transactionFixedTerm.getTypeTransaction()) {
                case DEPOSIT :
                  limit = fixedTerm.getLimitDeposits();
                  break;
                case DRAFT :
                  limit = fixedTerm.getLimitDraft();
                  break;
              }
              return count < limit && fixedTerm.getAllowDateTransaction()
                  .equals(LocalDate.now());
            }).flatMap(count -> {
              if (fixedTerm.getFreeTransactions() > count) {
                transactionFixedTerm.setCommissionAmount(0.0);
                fixedTerm.setBalance(fixedTerm.getBalance()
                    + transactionFixedTerm.getTransactionAmount());
              } else {
                transactionFixedTerm
                    .setCommissionAmount(fixedTerm.getCommissionTransactions());
                fixedTerm.setBalance(fixedTerm.getBalance()
                    + transactionFixedTerm.getTransactionAmount()
                    - fixedTerm.getCommissionTransactions());
              }
              return service.updateFixedTerm(fixedTerm).flatMap(ftUpdate -> {
  ...
  public Mono<ResponseEntity<Transaction>> update(
    ...
    return service
        .findFixedTermById(transactionFixedTerm.getFixedTerm().getId())
        .flatMap(sa -> {
          return service.findById(transactionFixedTerm.getId()).flatMap(sat -> {
            switch (transactionFixedTerm.getTypeTransaction()) {
              case DEPOSIT :
